...
import {AutomationRuleType,Prisma} from '@prisma/client';
...
@Injectable() export class AutomationService {
 ...
 async createRule(org:string,userId:string,dto:CreateAutomationRuleDto,request:Request){const exists=await this.db.automationRule.findFirst({where:{organizationId:org,name:dto.name.trim()}});if(exists)throw new BadRequestException('Une règle porte déjà ce nom.');const r=await this.db.automationRule.create({data:{organizationId:org,name:dto.name.trim(),type:dto.type as AutomationRuleType,enabled:dto.enabled??true,config:(dto.config??{}) as Prisma.InputJsonValue}});await this.audit.record({organizationId:org,userId,action:'CREATE',entity:'AutomationRule',entityId:r.id,next:r,ip:request.ip,userAgent:request.headers['user-agent']});return r;}
 async updateRule(org:string,userId:string,id:string,dto:Partial<CreateAutomationRuleDto>,request:Request){const before=await this.db.automationRule.findFirst({where:{id,organizationId:org}});if(!before)throw new NotFoundException('Règle introuvable.');const r=await this.db.automationRule.update({where:{id},data:{name:dto.name?.trim(),type:dto.type===undefined?undefined:dto.type as AutomationRuleType,enabled:dto.enabled,config:dto.config===undefined?undefined:dto.config as Prisma.InputJsonValue}});await this.audit.record({organizationId:org,userId,action:'UPDATE',entity:'AutomationRule',entityId:id,previous:before,next:r,ip:request.ip,userAgent:request.headers['user-agent']});return r;}
}
